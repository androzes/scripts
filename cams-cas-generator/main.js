...
const validateInputs = (event) => {
  ...
  return {email, password, fromDate, toDate, headless}
}

const run = async (inputs) => {
  debug('Original Inputs: %s', inputs);
  inputs = validateInputs(inputs);
  debug('Inputs: %s', inputs);

  const browser = await puppeteer.launch({
    headless: inputs.headless,
    devtools: !inputs.headless
  });
  ...
  if(datePickerFromDate) {
    ...
    await page.keyboard.type(inputs.fromDate, {delay:INPUT_DELAY_IN_MS});
  ...
  if(datePickerToDate) {
    ...
    await page.keyboard.type(inputs.toDate, {delay:INPUT_DELAY_IN_MS});
  ...
  if(inputEmail) {
    ...
    await page.keyboard.type(inputs.email, {delay:INPUT_DELAY_IN_MS});
  ...
  if(inputPassword) {
    ...
    await page.keyboard.type(inputs.password, {delay:INPUT_DELAY_IN_MS});
...
run({email, password, fromDate, toDate, headless});
